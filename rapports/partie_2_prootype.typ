...
- the tool currently selected (hghlighted in the interface).
- the layer currently selected

The tool enabled by default is the selection tool. When the user clicks on a key, it will open all it's metadata in the right pannel.

They are a few pop-up menus for specific features, like setting the keycode for a key.aa

== Geometry features

The user starts with the "crete new key" tool. Each time he clicks on the canvas, a new key is created.
...
There is also a "geomety" tool. It can translate, rotate and scale the keys.
...
The 2 properties are always visible on the interface. The user can change them in the right pannel.
...
When the user is happy with his keyboard, he can generate a visualisation. It consists in a svg image for each layer.
