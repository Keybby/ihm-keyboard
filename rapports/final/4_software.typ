...
- *K.I.S.S (Keep it simple, stupid)*. In particular, we did not want to use a heavy framework from NPM with too much features. This would slow down our app, take us time to understand how the library works and hide complexity. In comparison, HTML + CSS + vanilla js is something we understand quite well, there is no risk of an edge case because of the specific of how the library works.
...
- Javascript with #link("https://jsdoc.app/about-getting-started")[JsDoc] and the typescript compiler in JS mode. This allow us to check javascript statically, without having to learn Typescript.
- The #link("https://alpinejs.dev/")[Alpine.js] library for the declarative part.


== Logic of our app

The Alpine.js framework allows us to structure our app with the _Model View Controler_ (MVC) architecture.

#diagram(
	node-stroke: 1pt,
	spacing: (3em, 5em),
	node((1,0), [Model (Js data)], name: <model>),
	node((0,1), [Controler (Alpine)], name: <controler>),
...
We took a lot of time implementing the right kind of dragging behaviour.
...
This is done by two mechanisms: *collision resolution* and *snap behaviour*.


#let size = 2

#grid(
  columns: (1fr, 1fr), align: center, gutter: 20pt,
  [*Collision resolution*], [*Snap*],
  canvas({
    let offset-x = 1.5
    let offset-y = -1.2
    draw.rect((-size/2, -size/2), (size/2, size/2), fill: blue.lighten(80%), stroke: blue.darken(40%))
    draw.rect((size/2, -size/2+offset-y), (size+size/2, size/2+offset-y), fill: red.lighten(80%), stroke: red.darken(40%))
    draw.rect((-size/2+offset-x, -size/2+offset-y+0.2), (size/2+offset-x, size/2+offset-y+0.2), fill: red.lighten(80%).opacify(-70%), stroke: (paint: red.darken(40%), dash: "dashed"))
  ...
  canvas({
    ...
    let offset-y = 0.5
    draw.rect((-size/2, -size/2), (size/2, size/2), fill: blue.lighten(80%), stroke: blue.darken(40%))
    draw.rect((-size/2+offset-x, -size/2), (size/2+offset-x, size/2), fill: red.lighten(80%), stroke: red.darken(40%))
    draw.rect((-size/2+offset-x, -size/2+offset-y), (size/2+offset-x, size/2+offset-y), fill: red.lighten(80%).opacify(-70%), stroke: (paint: red.darken(40%), dash: "dashed"))
...
Collisions are known to be quite hard to compute in general, so we disigned an algorithm that works well in most cases.
...
#figure(caption: "the reviung41, a keyboard where the columns are aligned but tilted", image("images/reviung41.png", height:100pt))
...
There was a lot of debates on wether to chose a _tutorial system_ of a _hint system_.
...
At each moment in time, the interface displays the hint corresponding to the milestone with the highest proprity.
